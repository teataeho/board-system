		...
		<%@ include file="../include/footer.jsp" %>
			...
			<<<<<<< HEAD <script>
				...
				<%@ include file="../include/footer.jsp" %>
					...
					<<<<<<< HEAD // 아이디 중복 확인 비동기 요청 준비 const xhr=new XMLHttpRequest();=======<script>
						...
						<<<<<<< HEAD document.getElementById('emailSelection').onchange=function () { var
							userEmail2=document.getElementById('userEmail2'); var
							emailSelection=document.getElementById('emailSelection'); if (emailSelection.value==='direct' ) {
							userEmail2.disabled=false; userEmail2.value='' ; } else { userEmail2.disabled=true;
							userEmail2.value=emailSelection.value; } }; //인증번호 이메일 전송
							document.getElementById('emailCheckBtn').addEventListener('click', function () { const
							email=document.getElementById('userEmail1').value + '@' + document.getElementById('userEmail2') .value;
					...
					fetch(' ${pageContext.request.contextPath}/user/mailCheck?email=' + email)
						...
						.then(data => {
							...
							document.getElementById('emailCheck').disabled=false; }); });=======//인증번호 이메일 전송
							document.getElementById('emailCheckBtn').addEventListener('click', function () { const
							email=document.getElementById('userEmail1').value + '@' + document.getElementById('userEmail2') .value;
							console.log('완성된 email: ' + email);
		console.log(' email2: ' + document.getElementById(' userEmail2'));
			...
			.then(data => {
				...
				alert(' 인증번호가 전송되었습니다. 확인 후 입력란에 정확히 입력해주세요.'); // 인증번호를 입력할 수 있는 입력란 활성화
							...
							<<<<<<< HEAD /*제일 아래 로그인 버튼 누르면 로그인창 호출*/ document.getElementById('loginBtn').addEventListener('click',
								function () { window.location.href="${pageContext.request.contextPath}/user/userLogin" ; }); </script>
								=======
								/*비밀번호 형식 검사 스크립트*/
								...
								function categoryChange(e) {
								var addrDong_mapo = ["상암동", "성산동", "망원동", "연남동", "동교동", "서교동", "합정동", "상수동", "창전동", "신수동", "노고산동",
								"대흥동", "염리동",
								"용강동", "도화동", "공덕동", "아현동", "신공덕동"
								];
								var addrDong_seodaemun = ["북가좌동", "남가좌동", "홍은동", "홍제동", "연희동", "신촌동", "봉원동", "북아현동", "현저동", "천연동"];
